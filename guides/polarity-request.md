...
const suppressNotFound: OnApiErrorHook = async (
  error,
  response,
  requestOptions,
  userOptions
...
const logNetworkError: OnNetworkErrorHook = async (
  error,
  requestOptions,
  userOptions
) => {
...
The Polarity server provides a rate limiter instance to your integration via the context object. You can pass this limiter to `PolarityRequest` to throttle outgoing HTTP requests. The limiter must satisfy the `PolarityRequestLimiter` interface exported by this library (which only requires a `schedule` method):

```typescript
import { PolarityRequest, type PolarityRequestLimiter } from 'polarity-integration-utils';

// The limiter is provided by the Polarity server via the integration context
const limiter: PolarityRequestLimiter = context.limiter;
...
if (!response) {
  ...
  return {
    entity,
    data: { summary: ['API Limit Reached'], details: { apiLimitReached: true } }
  };
}
```
